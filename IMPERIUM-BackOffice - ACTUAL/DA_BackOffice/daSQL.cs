...
namespace DA_BackOffice
{
    public class daSQL
    {
        SqlConnection dbConn;
        readonly daEncrypta daEncryp;
        ...
        public object ExecuteScalarValor(string procedure, Dictionary<string, object> dictionary = null)
        {
            try
            {
                bool isProcedure = procedure.Contains("SELECT");
                if (dbConn.State == ConnectionState.Closed) { GetConnection(); }
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = procedure;
                cmd.CommandType = isProcedure ? CommandType.Text : CommandType.StoredProcedure;
                cmd.Connection = dbConn;

                if (dictionary != null) { agregarParametros(cmd, dictionary); }
