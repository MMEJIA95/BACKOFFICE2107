...
namespace UI_BackOffice.Formularios.Sistema.Configuracion_del_Sistema
{
    public partial class frmMantUsuario : HNG_Tools.ModalForm
    {
        ...
        public string cod_usuario = "",flg_aprobador="";
        ...
        private void GrabarAprobacionesVinculadas()
        {
            string multiple = "";
            string aproba = "";
            gvAprobaciones.RefreshData();

            for (int x = 0; x <= gvAprobaciones.RowCount - 1; x++)
            {
                eUsuario_Aprobacion eUserEmp = new eUsuario_Aprobacion();
                eUsuario_Aprobacion obj = gvAprobaciones.GetRow(x) as eUsuario_Aprobacion;

                if (obj.Seleccionado)
                {
                    obj.cod_usuario = cod_usuario;

                    multiple = aproba + "," + flg_aprobador;
                    obj.flg_aprobador = multiple + "," + flg_aprobador;
                    //eUserEmp = uow.Usuarios.Actualizar_UsuarioAprobaciones<eUsuario_Aprobacion>(obj);
                    //if (eUserEmp == null) { UI.ShowMessage("Error al Registrar Aprobación", "Aprobaciones", HNG_Tools.Interfaces.TypeMessage.Error); return; }
                }
            }
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    GrabarPerfiles();
                    if (MiAccion == Usuario.Nuevo) { GrabarEmpresasVinculadas(); GrabarSolucionesVinculadas(); GrabarAprobacionesVinculadas(); }
