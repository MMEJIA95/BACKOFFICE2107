...
namespace BL_BackOffice
{
    public class blUsuario
    {
        ...
        public T Actualizar_UsuarioAprobaciones<T>(int opcion,eUsuario_Aprobacion eUsu,string flg_aprobador="") where T : class, new()
        {
            Dictionary<string, object> dictionary = new Dictionary<string, object>()
            {
                { "opcion", opcion },
                { "cod_usuario", eUsu.cod_usuario },
                { "flg_aprobador", eUsu.flg_aprobador }
        ...
        public List<T> ListarAprobaciones<T>(int opcion, string cod_usuario = "") where T : class, new()
        {
            List<T> myList = new List<T>();
            Dictionary<string, object> dictionary = new Dictionary<string, object>() {
                { "opcion", opcion},
            ...
            myList = sql.ListaconSP<T>("usp_ConsultasVarias_Usuario", dictionary);
            return myList;
        }

        public T Insertar_Actualizar_Aprobaciones<T>( eUsuario_Aprobacion eUsu) where T : class, new()
        {
            Dictionary<string, object> dictionary = new Dictionary<string, object>()
            {
                { "cod_aprobacion", eUsu.cod_aprobacion },
                { "dsc_modulo", eUsu.dsc_modulo },
                { "imp_maximo", eUsu.imp_maximo },
                 { "cant_maxima", eUsu.cant_maxima },
                 { "descripcion", eUsu.descripcion },
                 { "cod_apro", eUsu.cod_apro }

            };

            T obj = sql.ConsultarEntidad<T>("usp_rhu_Insertar_Actualizar_Aprobaciones", dictionary);
            return obj;
        }
        public string Eliminar_UsuarioAprobaciones(int opcion, string flg_aprobador = "", int cod_pro=0)
        {
            Dictionary<string, object> dictionary = new Dictionary<string, object>();
            dictionary.Add("opcion", opcion);
            dictionary.Add("flg_aprobador", flg_aprobador);
            dictionary.Add("cod_apro", cod_pro);

            string result;
            result = sql.ExecuteScalarWithParams("usp_Insertar_Eliminar_UsuarioAprobaciones", dictionary);
            return result;
        }
